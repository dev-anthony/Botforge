...
function HeaderSection() {
  ...
  const linkClass =
    'text-sm font-medium text-[var(--color-gray-400)] hover:text-white transition-colors duration-200';

  return (
    <>
      {/* ── Desktop Header ───────────────────────── */}
      <header
        className="hidden md:flex fixed top-5 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-4xl items-center justify-between px-4 rounded-full border transition-all duration-500 ease-in-out"
        style={{
          background:       scrolled ? 'rgba(5,14,26,0.7)'  : 'rgba(255,255,255,0.04)',
          borderColor:      scrolled ? 'rgba(34,211,238,0.2)' : 'rgba(255,255,255,0.08)',
      ...
      <div className="md:hidden">
        {/* Fixed top bar */}
        <header
          className="w-[90%] fixed top-5 left-0 right-0 mx-auto z-50 flex items-center justify-between px-4 rounded-full border transition-all duration-500 ease-in-out"
          style={{
            background:     scrolled ? 'rgba(5,14,26,0.7)'   : 'rgba(255,255,255,0.04)',
            borderColor:    scrolled ? 'rgba(34,211,238,0.2)' : 'rgba(255,255,255,0.08)',
            backdropFilter: scrolled ? 'blur(16px)' : 'blur(4px)',
        ...
        {/* Mobile dropdown */}
        <nav
          className={`fixed w-[90%] top-24 left-4 right-4 z-40 rounded-2xl border border-black/10 dark:border-white/10
        ...
        >
          {navLinks.map((link) => (
            <a
              ...
              className="text-sm font-medium text-[var(--color-gray-400)] hover:text-white py-2 px-3 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-all"
            >
              {link.label}
            </a>
          ))}
