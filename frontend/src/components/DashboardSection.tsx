...
export default function DashboardSection() {
  ...
  const stats = [
    { title: 'Active Bots',       value: '12',     icon: Bot,          change: '+2 this week' },
    { title: 'Messages Today',    value: '1,247',  icon: MessageSquare, change: '+18% from yesterday' },
    { title: 'Response Rate',     value: '98.5%',  icon: TrendingUp,   change: 'Excellent' },
    { title: 'Avg Response Time', value: '0.5s',   icon: Zap,          change: 'Very fast' },
  ];

  const bots = [
    { name: 'Support Bot', platform: 'Telegram',  logo: '📱', status: 'offline', messages: 342 },
    { name: 'Sales Bot',   platform: 'WhatsApp',  logo: '💬', status: 'online',  messages: 589 },
    { name: 'Order Bot',   platform: 'WhatsApp',  logo: '🛒', status: 'online',  messages: 316 },
  ];

  return (
    <section className="relative w-full py-10 sm:py-14 md:py-16 px-2 sm:px-4 md:px-6">
      ...
      <div className="max-w-4xl mx-auto">
        <div
          data-aos="fade-up"
          data-aos-delay="100"
          className="p-4 sm:p-6 rounded-2xl border-2 border-[var(--color-cyan-400-30)] bg-gradient-to-br from-[var(--color-cyan-400-8)] via-[var(--color-cyan-400-2)] to-transparent backdrop-blur-xl shadow-2xl"
        >
          {/* Dashboard Header */}
          <div className="mb-4 pb-2 border-b border-[var(--color-cyan-400-20)]">
            <h3 className="text-base sm:text-lg font-semibold text-white mb-0.5">Dashboard Overview</h3>
            <p className="text-xs text-[var(--color-gray-400)]">Real-time analytics of your bot performance</p>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
            {stats.map((stat, idx) => (
              <div
                key={idx}
                className="p-3 rounded-lg border border-[var(--color-cyan-400-20)] bg-[var(--color-cyan-400-5)] hover:bg-[var(--color-cyan-400-10)] transition-all duration-300"
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <p className="text-xs text-[var(--color-gray-400)] mb-0.5">{stat.title}</p>
                    <p className="text-lg sm:text-xl font-bold text-white">{stat.value}</p>
                  </div>
                  <div className="p-1.5 rounded-lg bg-[var(--color-cyan-400-10)]">
                    <stat.icon className="w-4 h-4 text-[var(--color-cyan-400)]" />
                  </div>
                </div>
                <p className="text-xs text-[var(--color-cyan-400)]" style={{ opacity: 0.7 }}>{stat.change}</p>
              </div>
            ))}
          </div>

          {/* Analytics Chart */}
          <div className="mt-3 pt-3 border-t border-[var(--color-cyan-400-20)]">
            <div className="mb-2">
              <p className="text-sm font-semibold text-white mb-0.5">Message Analytics</p>
              <p className="text-xs text-[var(--color-gray-400)]">Incoming messages vs Outgoing responses</p>
            </div>

            {/* Legend */}
            <div className="flex flex-wrap gap-3 mb-2">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-[var(--color-cyan-400)]"></div>
                <span className="text-xs text-[var(--color-gray-400)]">Incoming</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-[var(--color-emerald-400)]"></div>
                <span className="text-xs text-[var(--color-gray-400)]">Outgoing</span>
              </div>
            </div>

            {/* Line Chart */}
            <div className="h-20 sm:h-24 rounded-lg bg-gradient-to-br from-[var(--color-cyan-400-5)] to-[var(--color-cyan-400-2)] border border-[var(--color-cyan-400-10)] p-4 flex items-end justify-between gap-1 relative">
              <div className="absolute inset-0 rounded-lg flex flex-col justify-between p-4 pointer-events-none">
                {[0, 1, 2, 3].map((i) => (
                  <div key={i} className="border-b border-[var(--color-cyan-400-5)] w-full"></div>
                ))}
              </div>

              <div className="absolute left-0 top-0 h-full w-10 flex flex-col justify-between text-xs text-[var(--color-gray-500)] p-4">
                <span>800</span>
                <span>400</span>
                <span>0</span>
              </div>
            ...
            <div className="flex justify-between text-xs text-[var(--color-gray-500)] mt-1 px-4">
          ...
          {/* Active Bots */}
          <div className="mt-3 pt-3 border-t border-[var(--color-cyan-400-20)]">
            <h4 className="text-base font-semibold text-white mb-2">Your Active Bots</h4>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {bots.map((bot, idx) => (
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <div className="text-lg">{bot.logo}</div>
                      <div>
                        <h4 className="text-sm font-semibold text-white">{bot.name}</h4>
                        <p className="text-xs text-[var(--color-gray-400)]">{bot.platform}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className={`w-2 h-2 rounded-full ${bot.status === 'online' ? 'bg-[var(--color-green-400)]' : 'bg-[var(--color-gray-400)]'}`}></div>
                      <p className="text-xs font-medium text-[var(--color-gray-400)] capitalize">{bot.status}</p>
                    </div>
                  </div>

                  <div className="h-px bg-gradient-to-r from-[var(--color-cyan-400-20)] to-transparent mb-2"></div>

                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <MessageCircle className="w-3 h-3 text-[var(--color-cyan-400)]" style={{ opacity: 0.7 }} />
                        <p className="text-xs text-[var(--color-gray-400)]">Messages</p>
                      </div>
                      <p className="text-sm font-bold text-white">{bot.messages}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="p-2 rounded bg-[var(--color-cyan-400-5)] border border-[var(--color-cyan-400-10)]">
                        <p className="text-xs text-[var(--color-gray-400)] mb-0.5">Response</p>
                        <p className="text-sm font-semibold text-[var(--color-cyan-400)]">98%</p>
                      </div>
                      <div className="p-2 rounded bg-[var(--color-cyan-400-5)] border border-[var(--color-cyan-400-10)]">
                        <p className="text-xs text-[var(--color-gray-400)] mb-0.5">Avg Time</p>
                        <p className="text-sm font-semibold text-[var(--color-cyan-400)]">0.3s</p>
                      </div>
                    </div>
                  </div>

                  <button className="w-full mt-2 py-1 px-3 rounded-lg text-xs font-medium text-[var(--color-cyan-400)] border border-[var(--color-cyan-400-30)] hover:bg-[var(--color-cyan-400-10)] transition-all duration-300">
                    View Details
                  </button>
                </div>
              ))}
